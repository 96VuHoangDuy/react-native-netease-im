...
RCT_EXPORT_METHOD(addReaction:(nonnull NSString *)messageId sessionId:(nonnull NSString *)sessionId sessionType:(nonnull NSString *)sessionType userId:(nonnull NSString *)userId reactionName:(nonnull NSString *)reactionName isSendNotification:(BOOL *)isSendNotification resolve:(RCTPromiseResolveBlock)resolve reject:(RCTPromiseRejectBlock)reject) {
     NSDictionary *result = [[ConversationViewController initWithConversationViewController] addReaction:messageId sessionId:sessionId sessionType:sessionType userId:userId reactionName:reactionName isSendNotification:isSendNotification];
    if (result != nil) {
        resolve(result);
    } else {
        reject(@"-1",@"Add reaction failed",nil);
    }
}
